//rafce (react arrow function component export autogeneration template)
...
const Login = () => {
  ...
  .then((userCredential) => {
    ...
    updateProfile(user, {
      displayName: name.current.value, photoURL: "https://img.freepik.com/premium-vector/smiley-face-chat-bubble-icon_701683-14.jpg?w=360",
    }).then(() => {
  ...
  return (
    ...
    <form onSubmit={(e)=> e.preventDefault()} className='w-3/12 absolute p-12 bg-black my-36 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-80'>
     ...
     {!isSignInForm && ( <input ref={name} type='text' placeholder='Full Name' className='p-4 my-4 w-full bg-gray-700'/>)}
      ...
      <button className='p-4 my-6 bg-red-700 w-full rounded-lg' onClick={handleFormSubmit}>{isSignInForm ? "Sign In" : "Sign Up"}</button>
